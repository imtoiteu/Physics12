# -*- coding: utf-8 -*-
"""16 hình Chương III – IV kế thừa từ bộ đề vận dụng cao (elite/), đổi tên theo
quy ước của bộ tài liệu này: t_ = từ trường, h_ = hạt nhân."""
import numpy as np
...
def t_sd_ray_nghieng():
    ...
    save(fig, "t_sd_ray_nghieng")
    return _reg("t_sd_ray_nghieng")
...
def t_dt_phi_t():
    ...
    save(fig, "t_dt_phi_t")
    return _reg("t_dt_phi_t")


# ---------------------------------------------------------------- 3. máy biến áp
def t_sd_may_bien_ap():
    ...
    save(fig, "t_sd_may_bien_ap")
    return _reg("t_sd_may_bien_ap")
...
def t_sd_khung_quay():
    ...
    ax.text(cx, -1.35, "vành khuyên và chổi quét", fontsize=8.6, color="#5a6470",
            ha="center")
    save(fig, "t_sd_khung_quay")
    return _reg("t_sd_khung_quay")


# ---------------------------------------------------------------- 6. thanh treo lò xo
def t_sd_thanh_lo_xo():
    ...
    save(fig, "t_sd_thanh_lo_xo")
    return _reg("t_sd_thanh_lo_xo")


# ---------------------------------------------------------------- 7. khung nghiêng với B
def t_sd_khung_nghieng():
    ...
    save(fig, "t_sd_khung_nghieng")
    return _reg("t_sd_khung_nghieng")
...
def h_dt_nllk_rieng():
    ...
    save(fig, "h_dt_nllk_rieng")
    return _reg("h_dt_nllk_rieng")


# ---------------------------------------------------------------- 9. khung vào vùng từ trường
def t_sd_khung_vao_B():
    ...
    ax.text(11.6, 3.25, "khung sau khi\nra khỏi vùng", fontsize=8.4, color=RED,
            ha="center")
    save(fig, "t_sd_khung_vao_B")
    return _reg("t_sd_khung_vao_B")


# ---------------------------------------------------------------- 10. nam châm rơi qua vòng dây
def t_sd_nam_cham_roi():
    ...
    for k, (x0, cap) in enumerate([(1.0, "nam châm lại gần"), (7.0, "nam châm ra xa")]):
        ...
        ax.text(x0 + 3.55, 2.0, "vòng dây\nkín", fontsize=8.4, color="#5a6470",
                ha="left", va="center")
    save(fig, "t_sd_nam_cham_roi")
    return _reg("t_sd_nam_cham_roi")
...
def t_sd_truyen_tai():
    ...
    save(fig, "t_sd_truyen_tai")
    return _reg("t_sd_truyen_tai")
...
def h_dt_H_t():
    ...
    fig.tight_layout()
    save(fig, "h_dt_H_t")
    return _reg("h_dt_H_t")


# ---------------------------------------------------------------- 13. cân dòng điện
def t_sd_can_dong_dien():
    ...
    ax.text(2.7, -1.05, "số chỉ của cân thay đổi khi có dòng điện", fontsize=9,
            color=RED, ha="center")
    save(fig, "t_sd_can_dong_dien")
    return _reg("t_sd_can_dong_dien")
...
def t_sd_ray_ngang():
    ...
    ax.text(3.1, -0.25, "ℓ = 40 cm (khoảng cách hai ray)", fontsize=8.8, color=GREEN,
            ha="center", va="top")
    save(fig, "t_sd_ray_ngang")
    return _reg("t_sd_ray_ngang")
...
def h_sd_phan_ra_po():
    ...
    save(fig, "h_sd_phan_ra_po")
    return _reg("h_sd_phan_ra_po")
...
def h_sd_tokamak():
    ...
    ax.text(0, -3.15, "cuộn dây tạo từ trường mạnh giam giữ plasma",
            fontsize=9, color=BLUE, ha="center")
    save(fig, "h_sd_tokamak")
    return _reg("h_sd_tokamak")


ALL = [t_sd_ray_nghieng, t_dt_phi_t, t_sd_may_bien_ap, t_dt_i_t, t_sd_khung_quay,
       t_sd_thanh_lo_xo, t_sd_khung_nghieng, h_dt_nllk_rieng, t_sd_khung_vao_B,
       t_sd_nam_cham_roi, t_sd_truyen_tai, h_dt_H_t, t_sd_can_dong_dien, t_sd_ray_ngang,
       h_sd_phan_ra_po, h_sd_tokamak]
