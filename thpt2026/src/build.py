...
def matrix_table(doc, group):
    P(doc, "MA TRẬN ĐỘ KHÓ CỦA BỘ 10 ĐỀ", bold=True, size=12.5,
      align=WD_ALIGN_PARAGRAPH.CENTER, before=6, after=3, color=DARKBLUE)
    rows = []
...
def rebalance(items):
    ...
    """
    numeric = [it for it in items if _numeric_options(it["o"])]
    pool = [it for it in items if not _numeric_options(it["o"])]
