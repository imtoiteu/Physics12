# -*- coding: utf-8 -*-
"""Sinh 4 tài liệu Word cho Vật lí 12 học kì II (Chương III và Chương IV)."""
...
from ch3_theory import CH3
from ch4_theory import CH4
from ch3_concept import MC3, DS3
from ch4_concept import MC4, DS4
from ch3_calc import CALC3
from ch4_calc import CALC4
from combined_calc import CALC34
...
def table(doc, caption, headers, rows):
    if caption:
        P(doc, caption, italic=True, size=10.5, align=WD_ALIGN_PARAGRAPH.CENTER, before=4, after=2)
...
def cover(doc, title_lines, subtitle, note):
    ...
    P(doc, "VẬT LÍ 12 – HỌC KÌ II", bold=True, size=19,
      align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
    P(doc, "Chương trình GDPT 2018 – Năm học 2026 – 2027", bold=True, size=12,
      align=WD_ALIGN_PARAGRAPH.CENTER, after=2)
    P(doc, "CHƯƠNG III: TỪ TRƯỜNG  •  CHƯƠNG IV: VẬT LÍ HẠT NHÂN", italic=True, size=11.5,
...
ORIENT = ("Tài liệu bám sát yêu cầu cần đạt của Chương trình GDPT 2018 và định dạng đề thi tốt nghiệp THPT "
          "từ năm 2025: Phần I gồm câu trắc nghiệm nhiều phương án lựa chọn, Phần II gồm câu trắc nghiệm "
          "đúng/sai (mỗi câu 4 ý), Phần III gồm câu trả lời ngắn.\n"
          "Trọng tâm đánh giá: hiểu bản chất hiện tượng, điều kiện áp dụng của các định luật, đọc hiểu đồ thị – "
          "bảng số liệu, suy luận thực nghiệm và vận dụng vào bối cảnh thực tiễn.")


# ----------------------------------------------------- đánh số thống nhất
def theory_structure():
    """Trả về [(tên phần, [(tên mục, [(mã câu, item)])])] cho tài liệu lí thuyết."""
    out = []
    for chap, mc, ds, tag, title in [
            (3, MC3, DS3, "3", "PHẦN A – CHƯƠNG III: TỪ TRƯỜNG"),
            (4, MC4, DS4, "4", "PHẦN B – CHƯƠNG IV: VẬT LÍ HẠT NHÂN")]:
        secs = []
        n = 0
...
def calc_structure():
    ...
    for data, tag, title in [
            (CALC3, "3", "PHẦN A – BÀI TẬP CHƯƠNG III: TỪ TRƯỜNG"),
            (CALC4, "4", "PHẦN B – BÀI TẬP CHƯƠNG IV: VẬT LÍ HẠT NHÂN"),
            (CALC34, "34", "PHẦN C – BÀI TẬP TỔNG HỢP CHƯƠNG III + CHƯƠNG IV")]:
        secs = []
        n = 0
...
def _numeric_options(opts):
    """True nếu bộ phương án là các giá trị số (giữ nguyên thứ tự cho dễ đọc)."""
    n = 0
    for o in opts:
...
def rebalance_answers(*datasets):
    ...
    Chỉ hoán vị vị trí các phương án trong cùng một câu (nội dung không đổi) và
    chỉ áp dụng cho những câu có phương án dạng chữ, để các câu đáp số giữ nguyên
    thứ tự tăng dần vốn dễ đọc hơn."""
    pool, counts = [], {L: 0 for L in LETTERS}
    for data in datasets:
        for _lvl, items in data.items():
            for it in items:
                if "o" in it:
                    counts[it["a"]] += 1
                    if not _numeric_options(it["o"]):
                        pool.append(it)
...
def render_question(doc, ident, item, show_answer=False):
    ...
    if k == "ds":
        rich(doc, [(ident + ". ", True, False, None), (item["stem"], False, False, None)],
             before=7, after=2)
        if item.get("fig"):
            picture(doc, item["fig"], maxw=11.5, maxh=7.2)
...
def render_solution(doc, ident, item):
    ...
    if k == "ds":
        rich(doc, [(ident + ". ", True, False, None), (item["stem"], False, True, None)],
             before=7, after=2)
        if item.get("fig"):
            picture(doc, item["fig"], maxw=10.5, maxh=6.6)
...
def build_teaching():
    ...
    cover(doc,
          ["FILE 1 – TÀI LIỆU DẠY HỌC", "LÍ THUYẾT CHƯƠNG III VÀ CHƯƠNG IV"],
          "Dùng cho giáo viên giảng dạy trên lớp và học sinh hệ thống hoá kiến thức",
          ORIENT)
    doc.add_page_break()

    for blocks in (CH3, CH4):
        for blk in blocks:
            t = blk[0]
            if t == "h1":
                ...
                box(doc, "★ LƯU Ý KHI ÔN THI", blk[1], fill="F2F8F0",
                    tcolor=RGBColor(0x1E, 0x64, 0x36))
            elif t == "fig":
    ...
    path = os.path.join(ROOT, "FILE1_TAI_LIEU_DAY_HOC_CHUONG_3_VA_4.docx")
    doc.save(path)
    return path
...
def build_exercises(structure, filename, title_lines, subtitle, extra_note=None):
    ...
    if extra_note:
        box(doc, "HƯỚNG DẪN SỬ DỤNG", extra_note, fill="F2F8F0",
            tcolor=RGBColor(0x1E, 0x64, 0x36))
    doc.add_page_break()
    total = 0
...
def build_solutions(theory_struct, calc_struct):
    ...
    path = os.path.join(ROOT, "FILE4_DAP_AN_VA_LOI_GIAI_CHI_TIET.docx")
    doc.save(path)
    return path
...
if __name__ == "__main__":
    ...
    for data in (MC3, MC4, CALC3, CALC4, CALC34):
        for _l, items in data.items():
            for it in items:
                if "o" in it:
                    before[it["a"]] += 1
    print("Phân bố đáp án trước khi cân bằng:", before)
    print("Phân bố đáp án sau khi cân bằng:  ", rebalance_answers(MC3, MC4, CALC3, CALC4, CALC34))
    ...
    p2, n2 = build_exercises(
        ths, "FILE2_BAI_TAP_LI_THUYET_CHUONG_3_VA_4.docx",
        ["FILE 2 – BÀI TẬP LÍ THUYẾT", "CHƯƠNG III VÀ CHƯƠNG IV"],
        ...
        "Phần câu hỏi đúng/sai ở cuối mỗi chương mô phỏng đúng dạng câu hỏi Phần II của đề thi tốt nghiệp THPT: "
        "mỗi câu có bốn ý, thí sinh phải đánh giá đúng hoặc sai cho từng ý.")
    print("Đã tạo:", os.path.basename(p2), "-", n2, "câu")

    p3, n3 = build_exercises(
        cas, "FILE3_BAI_TAP_TINH_TOAN_VA_SUY_LUAN.docx",
        ["FILE 3 – BÀI TẬP TÍNH TOÁN VÀ SUY LUẬN", "CHƯƠNG III, CHƯƠNG IV VÀ TỔNG HỢP"],
